 ...
 */
    bool test_trivial() {
        accessor<0, range<0,0,0,0>, 4> first(3,2,-1,5);
        std::cout<<first.get<3>()<<" "<<first.get<2>()<<" "<<first.get<0>()<<" "<<first.get<0>()<<std::endl;
        return first.get<3>()==3 && first.get<2>()==2 && first.get<1>()==-1 && first.get<0>()==5 ;
