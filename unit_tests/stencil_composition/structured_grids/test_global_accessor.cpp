...
class global_accessor_single_stage : public ::testing::Test {
  public:
    global_accessor_single_stage()
        : sinfo(10, 10, 10), sol_(sinfo, 2.), bd(20), bd_(make_global_parameter<target_t>(bd)), di(1, 0, 1, 9, 10),
  ...
  protected:
    ...
    data_store_t sol_;
    boundary bd;
    decltype(make_global_parameter<target_t>(bd)) bd_;
...
TEST_F(global_accessor_single_stage, boundary_conditions) {
    ...
    auto bc_eval = make_computation<target_t>(coords_bc,
        p_sol() = sol_,
        p_bd() = bd_,
...
TEST(test_global_accessor, multiple_stages) {
    ...
    auto bd_ = make_global_parameter<target_t>(bd);
