...
namespace gridtools {
    ...
    TEST(tuple, element_wise_ctor) {
        tuple<int, double> testee = on_device::exec(MAKE_CONSTANT(element_wise_ctor), 42, 2.5);
    ...
    TEST(tuple, element_wise_conversion_ctor) {
        tuple<int, double> testee = on_device::exec(MAKE_CONSTANT(element_wise_conversion_ctor), 'a', 'b');
    ...
    TEST(tuple, tuple_conversion_ctor) {
        tuple<int, double> testee = on_device::exec(MAKE_CONSTANT(tuple_conversion_ctor), tuple<char, char>{'a', 'b'});
        EXPECT_EQ('a', tuple_util::host::get<0>(testee));
        EXPECT_EQ('b', tuple_util::host::get<1>(testee));
    }
} // namespace gridtools
