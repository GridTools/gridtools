...
struct verifier<int>
{
    static void TestEQ(int val, int exp) {
        ASSERT_EQ(val, exp);
    }
};
...
template<typename T>
__global__
void atomic_add_kernel(T* pReduced, const T* field, const int size)
{
...
template<typename T>
__global__
void atomic_sub_kernel(T* pReduced, const T* field, const int size)
{
    const int i = threadIdx.x + blockIdx.x*blockDim.x;
    const int j = threadIdx.y + blockIdx.y*blockDim.y;
    const int pos = j*gridDim.x*blockDim.x + i;
...
template<typename T>
__global__
void atomic_min_kernel(T* pReduced, const T* field, const int size)
{
    const int i = threadIdx.x + blockIdx.x*blockDim.x;
    const int j = threadIdx.y + blockIdx.y*blockDim.y;
    const int pos = j*gridDim.x*blockDim.x + i;
...
template<typename T>
__global__
void atomic_max_kernel(T* pReduced, const T* field, const int size)
{
    const int i = threadIdx.x + blockIdx.x*blockDim.x;
    const int j = threadIdx.y + blockIdx.y*blockDim.y;
    const int pos = j*gridDim.x*blockDim.x + i;
...
TEST(AtomicFunctionsUnittest, atomic_add_int)
{
    test_atomic_add<int>();
}
...
TEST(AtomicFunctionsUnittest, atomic_sub_int)
{
    test_atomic_sub<int>();
}
...
TEST(AtomicFunctionsUnittest, atomic_min_int)
{
    test_atomic_min<int>();
}
...
TEST(AtomicFunctionsUnittest, atomic_max_int)
{
    test_atomic_max<int>();
}
