...
template <>
struct static_type_tests<backend<target::cuda>> {
...
struct static_type_tests<backend<Target>> {
...
TEST(StorageFacility, ViewTests) {
    typedef storage_traits<backend_t::backend_target_t>::storage_info_t<0, 3> storage_info_ty;
    typedef storage_traits<backend_t::backend_target_t>::data_store_t<double, storage_info_ty> data_store_t;
...
template <class BackendTarget>
struct static_layout_test_cases {
    using layout1_t = typename storage_traits<BackendTarget>::template storage_info_t<0, 1>::layout_t;
    using layout2_t = typename storage_traits<BackendTarget>::template storage_info_t<0, 2>::layout_t;
    using layout3_t = typename storage_traits<BackendTarget>::template storage_info_t<0, 3>::layout_t;
    using layout4_t = typename storage_traits<BackendTarget>::template storage_info_t<0, 4>::layout_t;
    using layout5_t = typename storage_traits<BackendTarget>::template storage_info_t<0, 5>::layout_t;

    using layout_s5_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 1, 1, 1>>::layout_t;
    using layout_s51_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 1, 1, 1, 1>>::layout_t;
    using layout_s52_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 0, 1, 1, 1>>::layout_t;
    using layout_s53_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 0, 1, 1>>::layout_t;
    using layout_s54_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 1, 0, 1>>::layout_t;
    using layout_s55_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 1, 1, 0>>::layout_t;

    using layout_s56_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 0, 1, 1, 1>>::layout_t;
    using layout_s57_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 0, 0, 1, 1>>::layout_t;
    using layout_s58_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 0, 0, 1>>::layout_t;
    using layout_s59_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 1, 0, 0>>::layout_t;

    using layout_s510_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 1, 0, 1, 1>>::layout_t;
    using layout_s511_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 0, 1, 0, 1>>::layout_t;
    using layout_s512_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 0, 1, 0>>::layout_t;

    using layout_s513_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 1, 1, 0, 1>>::layout_t;
    using layout_s514_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 0, 1, 1, 0>>::layout_t;

    using layout_s515_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 1, 1, 1, 0>>::layout_t;

    using layout_s516_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 0, 0, 1, 1>>::layout_t;
    using layout_s517_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 0, 0, 0, 1>>::layout_t;
    using layout_s518_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 1, 0, 0, 0>>::layout_t;
    using layout_s519_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 1, 1, 0, 0>>::layout_t;
    using layout_s520_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 0, 1, 1, 0>>::layout_t;

    using layout_s521_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<1, 0, 0, 0, 0>>::layout_t;
    using layout_s522_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 1, 0, 0, 0>>::layout_t;
    using layout_s523_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 0, 1, 0, 0>>::layout_t;
    using layout_s524_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 0, 0, 1, 0>>::layout_t;
    using layout_s525_t =
        typename storage_traits<BackendTarget>::template special_storage_info_t<0, selector<0, 0, 0, 0, 1>>::layout_t;
};

template <class BackendTarget>
struct static_layout_tests_decreasing : static_layout_test_cases<BackendTarget> {
    using cases = static_layout_test_cases<BackendTarget>;
...
template <class BackendTarget>
struct static_layout_tests_decreasing_swappedxy : static_layout_test_cases<BackendTarget> {
    using cases = static_layout_test_cases<BackendTarget>;
    GT_STATIC_ASSERT((boost::is_same<typename cases::layout1_t, layout_map<0>>::value), "layout type is wrong");
    GT_STATIC_ASSERT((boost::is_same<typename cases::layout2_t, layout_map<1, 0>>::value), "layout type is wrong");
...
template <class BackendTarget>
struct static_layout_tests_increasing : static_layout_test_cases<BackendTarget> {
    using cases = static_layout_test_cases<BackendTarget>;
    GT_STATIC_ASSERT((boost::is_same<typename cases::layout1_t, layout_map<0>>::value), "layout type is wrong");
...
#ifdef __CUDACC__
template <>
struct static_layout_tests<backend<target::cuda>> : static_layout_tests_decreasing<target::cuda> {};
#endif
...
template <>
struct static_layout_tests<backend<target::x86>> : static_layout_tests_increasing<target::x86> {};
...
TEST(StorageFacility, CustomLayoutTests) {
    typedef typename storage_traits<backend_t::backend_target_t>::custom_layout_storage_info_t<0,
        layout_map<2, 1, 0>>::layout_t layout3_t;
    typedef typename storage_traits<backend_t::backend_target_t>::custom_layout_storage_info_t<0,
        layout_map<1, 0>>::layout_t layout2_t;
    typedef
        typename storage_traits<backend_t::backend_target_t>::custom_layout_storage_info_t<0, layout_map<0>>::layout_t
            layout1_t;
    typedef typename storage_traits<backend_t::backend_target_t>::custom_layout_storage_info_t<0,
