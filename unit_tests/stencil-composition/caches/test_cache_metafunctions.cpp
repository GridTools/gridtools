...
struct functor1 {
    ...
    typedef accessor<1, enumtype::inout, extend<0,0,0,0>, 5> buff;
...
TEST(cache_metafunctions, extract_extends_for_caches)
{
    ...
    typedef boost::mpl::vector2< extend<-1,2,-2,1>, extend<-2,1,-3,2> > extends_t;
    ...
    typedef extract_extends_for_caches<iterate_domain_arguments_t>::type extends_map_t;

    GRIDTOOLS_STATIC_ASSERT((boost::mpl::equal<extends_map_t,
            boost::mpl::map3<
                boost::mpl::pair<cache1_t, extend<-1,2,-2,1> >,
                boost::mpl::pair<cache2_t, extend<-2,2,-3,2> >,
                boost::mpl::pair<cache3_t, extend<-2,1,-3,2> >
            > >::value), "ERROR"
    );
}
...
TEST(cache_metafunctions, get_cache_storage_tuple)
{
    ...
    typedef get_cache_storage_tuple<IJ, caches_t, extends_map_t, block_size<32,4>, local_domain_t>::type cache_storage_tuple_t;
    ...
    GRIDTOOLS_STATIC_ASSERT((
        boost::mpl::equal<
            ...
            boost::fusion::map<
                boost::fusion::pair<p_in::index_type, cache_storage<float_type, block_size<32,4>, extend<-1,2,-2,1> > >,
                boost::fusion::pair<p_buff::index_type, cache_storage<float_type, block_size<32,4>, extend<-2,2,-3,2> > >
            >
        >::value),"ERROR");
}
