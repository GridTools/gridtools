...
namespace test_cycle_and_swap{
    ...
    struct functor{
        typedef accessor<0, enumtype::inout,  extent<>, 4> p_i;
    ...
    bool test_3D(){
        ...
        for(uint_t k=0; k < d3; ++k)
        {
            for(uint_t i=halo_size; i < d1-halo_size; ++i)
            {
                ...
                {
                    reference.get_value<0,0>(i,j,k) =  (i_data.get_value<0,0>(i+1,j,k) + i_data.get_value<0,0>(i-1,j,k))*(float_t)0.5;
                }
            }
        ...
        //note that the second run will do wrong computations at the first line of the 2D domain of the coordinates, because the first line of
        // grid points at the boundaries has not been computed at the first run. However we just dont validate these points with the verifier
        comp->run();
        comp->finalize();
