...
namespace vs_test{
    ...
    struct test_on_vertexes_functor {
        typedef in_accessor<0, icosahedral_topology_t::vertexes, radius<1> > in;
        typedef inout_accessor<1, icosahedral_topology_t::vertexes> out;
        typedef in_accessor<2, icosahedral_topology_t::vertexes, radius<1> > ipos;
        typedef in_accessor<3, icosahedral_topology_t::vertexes, radius<1> > cpos;
        typedef in_accessor<4, icosahedral_topology_t::vertexes, radius<1> > jpos;
        typedef in_accessor<5, icosahedral_topology_t::vertexes, radius<1> > kpos;
...
TEST(test_stencil_on_vertexes, run) {
    ...
    EXPECT_TRUE(ver.verify(ref_vertexes, out_vertexes, halos));
}
