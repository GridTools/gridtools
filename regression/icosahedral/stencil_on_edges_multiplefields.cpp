...
struct test_on_edges_functor {
    GT_DEFINE_ACCESSORS(GT_IN_ACCESSOR(in1, enumtype::edges, extent<1, -1, 1, -1>),
        GT_IN_ACCESSOR(in2, enumtype::edges, extent<1, -1, 1, -1>),
        GT_INOUT_ACCESSOR(out, enumtype::edges));
...
TEST_F(stencil_on_edges_multiplefields, test) {
    ...
    auto comp = make_computation(p_in1 = make_storage<edges>(in1),
        ...
        p_out = out,
        make_multistage(execute::forward(), make_stage<test_on_edges_functor, topology_t, edges>(p_in1, p_in2, p_out)));
    comp.run();
