        ...
        GT_FUNCTION static void Do(Evaluation const &eval, x_interval) {
            ...
            for (uint_t i = 0; i < 3; ++i) {
                eval(weight_edges(edge + i)) = eval(cell_area()) * (1 + (float_type)1.0 / (float_type)(i + 1));
                                std::cout << "REF  " << i << " " << eval(weight_edges(edge + i)) << " " << &eval(weight_edges(edge + i)) << std::endl;
            }

            //            if (eval.position()[1] == 0) {
            //                constexpr auto offsets = from< cells >::to< cells >::with_color< static_int< 0 >
            //                >::offsets();
            //                for (auto offset : offsets) {
            //                    array< uint_t, 4 > coffsets;
            //                    for (int i = 0; i < 4; i++)
            //                        coffsets[i] = offset[i];

            //                    //                    eval(weight_edges()) =
            //                    //                            eval._evaluate(cell_area(), coffsets);
            //                    std::cout << "PPP " << coffsets[0] << " " << coffsets[1] << " " << coffsets[2] << " "
            //                    << coffsets[3] << std::endl;
            //                    eval(weight_edges()) = 10;
            ////                            eval._evaluate(cell_area(), coffsets);
            //                }
            //            } else if (eval.position()[1] == 1) {
            //                constexpr auto offsets = from< cells >::to< cells >::with_color< static_int< 1 >
            //                >::offsets();
            //                for (auto offset : offsets) {
            //                    array< uint_t, 4 > coffsets;
            //                    for (int i = 0; i < 4; i++)
            //                        coffsets[i] = offset[i];
            //                    eval(weight_edges()) = eval(cell_area(offset));
            //                    //                    eval(weight_edges()) =
            //                    //                            eval._evaluate(cell_area(), coffsets);
            //                }
            //            }
