...
// This is the definition of the special regions in the "vertical" direction
    ...
    struct cox_de_bohr{
        ...
        uint_t g;
        constexpr cox_de_bohr(uint_t g_) : g(g_) {}
    ...
    struct apply_expr{
        uint_t gx, gy;
        constexpr apply_expr(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}
        ...
        static constexpr type value = Expression<K1, K2>(gx, gy)::value;
    };

    template<uint_t K1, template <uint_t L, uint_t K2> class Expression>
    struct apply_expr<K1, 0, Expression>{
        constexpr apply_expr(uint_t gx_, uint_t gy_) {}
    ...
    struct evaluate{
        ...
        template <int_t I>
        using N=cox_de_bohr<P, I, enumtype::Dimension<3>, in, knots>;

        //basis function in y direction
        template <int_t I>
        using M=cox_de_bohr<Q, I, enumtype::Dimension<4>, in, knots>;
        ...
        struct inner_expr_x {
            uint_t gx, gy;
            constexpr inner_expr_x(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}

            template <uint_t U, uint_t V>
            using dummy=inner_expr_x<U,V>;
            static constexpr auto value = ((N<-P+ k1 -1>(gx).expr * M<-Q+ k2 -1>(gy).expr * points(x(-P+k1-1), y(-Q+k2-1) ) ) + apply_expr<k1-1, k2, dummy>::value);
        };
        template<uint_t k1, uint_t k2>
        struct inner_expr_y {
            uint_t gx, gy;
            constexpr inner_expr_y(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}

            template <uint_t U, uint_t V>
            using dummy=inner_expr_y<U,V>;
            static constexpr auto value = ((N<-P+ k1 -1>(gx).expr * M<-Q+ k2 -1>(gy).expr * points(x(-P+k1-1), y(-Q+k2-1), comp(1)) ) + apply_expr<k1-1, k2, dummy>::value);
        };
        template<uint_t k1, uint_t k2>
        struct inner_expr_z {
            uint_t gx, gy;
            constexpr inner_expr_z(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}

            template <uint_t U, uint_t V>
            using dummy=inner_expr_z<U,V>;
            static constexpr auto value = ((N<-P+ k1 -1>(gx).expr * M<-Q+ k2 -1>(gy).expr * points(x(-P+k1-1), y(-Q+k2-1), comp(2)) )  + apply_expr<k1-1, k2, dummy>::value);
        };
        ...
        struct expr_x{
            uint_t gx, gy;
            constexpr expr_x(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}

            template <uint_t U, uint_t V>
            using dummy=expr_x<U,V>;
            static constexpr auto value = inner_expr_x<k1, k2>(gx,gy).value + apply_expr<k1, k2-1, dummy>(gx,gy).value;
        };

        template< uint_t k1, uint_t k2 >
        struct expr_y{
            uint_t gx, gy;
            constexpr expr_y(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}

            template <uint_t U, uint_t V>
            using dummy=expr_y<U,V>;
            static constexpr auto value = inner_expr_y<k1, k2>(gx,gy).value + apply_expr< k1, k2-1, dummy>(gx,gy).value;
        };

        template< uint_t k1, uint_t k2 >
        struct expr_z{
            uint_t gx, gy;
            constexpr expr_z(uint_t gx_, uint_t gy_) : gx(gx_), gy(gy_) {}

            template <uint_t U, uint_t V>
            using dummy=expr_z<U,V>;
            static constexpr auto value = inner_expr_z<k1, k2>(gx,gy).value + apply_expr< k1, k2-1, dummy>(gx,gy).value;
        };
    ...
    struct compute_functor{
        ...
        public:
            ...
            void operator() (GY /**/){
                using evaluate_t=evaluate<P,Q>;
                m_eval(out())    =m_eval(apply_expr<P+1, Q+1, evaluate_t::template expr_x>::type(i,j));
                m_eval(out(z(1)))=m_eval(apply_expr<P+1, Q+1, evaluate_t::template expr_y>::type(i,j));
                m_eval(out(z(2)))=m_eval(apply_expr<P+1, Q+1, evaluate_t::template expr_z>::type(i,j));
            }
        };
        ...
        struct eval_functor{
            ...
            //     for_each<boost::mpl::range_c<uint_t,0,GX> >(outer_loop_functor<Evaluation, GY>(eval));
                ...
                for(uint_t i=0; i<GX; ++i)
                    for(uint_t j=0; j<GY; ++j)
                    {

                        evaluate evaluate_={P,Q,i,j};
                        eval(out())    =eval(apply_expr<P+1, Q+1, evaluate_t::template expr_x>::value);
                        eval(out(z(1)))=eval(apply_expr<P+1, Q+1, evaluate_t::template expr_y>::value);
                        eval(out(z(2)))=eval(apply_expr<P+1, Q+1, evaluate_t::template expr_z>::value);
                    }
