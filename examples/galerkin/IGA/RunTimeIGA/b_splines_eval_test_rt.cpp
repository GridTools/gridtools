...
constexpr std::array<double, NK> knots {0,1,2,3,4,5,6};

// Non-GT type b-spline instance allocation
constexpr int I(2); // b-spline index
iga_rt::BSpline<I,P> bspline(knots);
...
struct bspline_struct
{
    ...
    typedef accessor<0, gridtools::enumtype::inout, extent<0, 0, 0, 0>, 3 > bsline_values;
...
int main()
{
    ...
    storage_type_bspline_basis_info basis_info(numPoints,1,1);
    storage_type_bspline_basis_values bspline_basis_values(basis_info);
    ...
    typedef gridtools::arg<1,storage_type_bspline_basis_values> p_bspline_values;
    ...
    boost::shared_ptr<gridtools::computation> bspline_calculation =
    		...
    		gridtools::make_computation<gridtools::BACKEND>(gridtools::make_multistage(gridtools::enumtype::execute<gridtools::enumtype::forward>(),
                                                                                    gridtools::make_stage<bspline_struct>(p_bspline_values(),
                                                                                                                        p_csi())),
                                                                domain,
                                                                coordinates);
    ...
    for(unsigned int csiIndex=0;csiIndex<numPoints;++csiIndex)
	{
		if(bspline.evaluate(csiValues[csiIndex])!=bspline_basis_values(csiIndex,0,0))
		{
			std::cout<<bspline.evaluate(csiValues[csiIndex])<<" "<<bspline_basis_values(csiIndex,0,0)<<std::endl;
		}
	}
