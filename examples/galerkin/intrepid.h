...
namespace intrepid{
    ...
    bool test(){
        ...
        typedef storage4::storage_t::layout layout_local_grid;
        ...
        storage2::storage_t  local_grid_reordered_s(geo_map::basisCardinality, geo_map::spaceDim);
        storage2 local_grid_reordered(local_grid_reordered_s, 2);
        // for(uint_t i=0; i<fe::basisCardinality; ++i){
        uint_t D=geo_map::basisCardinality;//get this from the order of the basis
        for(uint_t i=0; i<D; ++i){//few redundant loops
            // for(uint_t j=0; j<D; ++j){
            //     for(uint_t k=0; k<D; ++k){
            {
                local_grid_reordered(i, 0)=local_grid(permutations[i],0);
                local_grid_reordered(i, 1)=local_grid(permutations[i],1);
                local_grid_reordered(i, 2)=local_grid(permutations[i],2);
        ...
        storage3::storage_t cub_points_s(cubature::numCubPoints, fe::spaceDim, 1);
        storage3::storage_t cub_weights_s(cubature::numCubPoints, 1, 1);
        storage3 cub_points(cub_points_s, 2);
        storage3 cub_weights(cub_weights_s, 1);
        storage3::storage_t grad_at_cub_points_s(fe::basisCardinality, cubature::numCubPoints, fe::spaceDim);
        storage3 grad_at_cub_points(grad_at_cub_points_s);

        // assert(fe::basisCardinality%fe::spaceDim==0);
        // const uint_t n_dofs_per_dim=fe::basisCardinality/fe::spaceDim;
        // storage_t<0,1,2,3,4>::storage_t gt_storage_grads( n_dofs_per_dim, n_dofs_per_dim, n_dofs_per_dim,
        //                                                   fe::numCubPoints, fe::spaceDim,
        //                                                   grad_at_cub_points_s.fields()[0].get());

        // retrieve cubature points and weights
        cubature::cub->getCubature(cub_points, cub_weights);
        ...
        jacobian_type::storage_t jac_s((d1*d2*d3), cubature::numCubPoints, 3, 3);
        ...
        typedef intrepid_storage<gridtools::BACKEND::storage_type<float_type, layout_jac_det_t >::type > jacobian_det_type;
         jacobian_det_type::storage_t jac_det_s((d1*d2*d3), cubature::numCubPoints);
