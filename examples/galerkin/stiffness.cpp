...
int main(){
    ...
    using bd_discr_t = intrepid::boundary_discr<bd_rule_t>;
    using bd_cub = typename bd_cub_t::bd_cub;
    bd_cub_t bd_cub_;
    bd_discr_t bd_discr_(bd_rule_, 0);
    ...
    using as=assembly< bd_points_t, geo_t >;


    //![as_instantiation]
    //constructing the integration tools
    as assembler(bd_discr_, geo, d1, d2, d3);
    //![as_instantiation]
    ...
    */
    ...
    //output
    typedef arg<as::size+5, matrix_type> p_flux;
    ...
    auto computation=make_computation<gridtools::BACKEND, gridtools::layout_map<0,1,2,3> >(
        ...
        (
            execute<forward>(),
            make_esf<as::update_jac >( as::p_grid_points(), as::p_jac(), p_dphi())
