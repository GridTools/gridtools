...
#ifdef CUDA_EXAMPLE
typedef gridtools::backend<gridtools::enumtype::Cuda, gridtools::enumtype::GRIDBACKEND, gridtools::enumtype::Block > hd_backend;
#else
#ifdef BACKEND_BLOCK
typedef gridtools::backend<gridtools::enumtype::Host, gridtools::enumtype::GRIDBACKEND, gridtools::enumtype::Block > hd_backend;
#else
typedef gridtools::backend<gridtools::enumtype::Host, gridtools::enumtype::GRIDBACKEND, gridtools::enumtype::Naive > hd_backend;
#endif
#endif
...
class repository
{
    ...
    repository(const uint_t idim, const uint_t jdim, const uint_t kdim, const uint_t halo_size):
        ...
        m_storage_info_j(idim, jdim, kdim),
        m_storage_info_scalar(idim, jdim, kdim),
        in_(m_storage_info_ijk, -1., "in"),
        crlato_(m_storage_info_j, -1, "crlato"),
        crlatu_(m_storage_info_j, -1, "crlatu"),
        crlat0_(m_storage_info_j, -1, "crlat0"),
        crlat1_(m_storage_info_j, -1, "crlat1"),
    ...
    void generate_reference_simple()
    {
        ...
        for(uint_t k=0; k < kdim_; ++k)
        {
            for (uint_t i = halo_size_-1; i < idim_-halo_size_+1; ++i)
            {
                for (uint_t j = halo_size_-1; j < jdim_-halo_size_+1; ++j)
                {
                    lap(i,j,(uint_t)0) = in_(i+1,j,k) + in_(i-1,j,k) - (gridtools::float_type)2*in_(i,j,k)
