...
namespace assembly{
    ...
    struct integration {
        typedef in_accessor<0, extend<-1, 1, -1, 1> , 4>  phi;
        typedef in_accessor<1, extend<-1, 1, -1, 1> , 4>  psi;//how to detect when index is wrong??
        typedef in_accessor<2, extend<-1, 1, -1, 1> , 4>  jac;
        typedef in_accessor<3, extend<-1, 1, -1, 1> , 6>  f;
        typedef inout_accessor<4, extend<-1, 1, -1, 1>, 6 > result;
