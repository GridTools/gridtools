...
struct array {
    ...
    array(T *_p, int _n, int _m, int _l)
        : ptr(_p), n(lmap::template find< 2 >(_n, _m, _l)), m(lmap::template find< 1 >(_n, _m, _l)),
          l(lmap::template find< 0 >(_n, _m, _l)) {}

    T &operator()(int i, int j, int k) {
        // a[(DIM1+2*H)*(DIM2+2*H)*kk+ii*(DIM2+2*H)+jj]
        return ptr[l * m * lmap::template find< 2 >(i, j, k) + l * lmap::template find< 1 >(i, j, k) +
...
struct triple_t {
    ...
    __host__ __device__ triple_t(triple_t const &t) : _x(t._x), _y(t._y), _z(t._z) {}
...
std::ostream &operator<<(std::ostream &s, triple_t const &t) {
    return s << " (" << t._x << ", " << t._y << ", " << t._z << ") ";
}

bool operator==(triple_t const &a, triple_t const &b) { return (a._x == b._x && a._y == b._y && a._z == b._z); }
...
struct triple_t {
    ...
    __host__ __device__ triple_t(triple_t const &t) : value(t.value) {}
...
triple_t operator*(int a, triple_t const &b) { return triple_t(a * b.x(), a * b.y(), a * b.z()); }
    ...
    for (int ii = H1m; ii < DIM1 + H1m; ++ii)
        for (int jj = H2m; jj < DIM2 + H2m; ++jj)
            for (int kk = H3m; kk < DIM3 + H3m; ++kk) {
                a(ii, jj, kk) =
                    triple_t(ii - H1m + (DIM1)*coords[0], jj - H2m + (DIM2)*coords[1], kk - H3m + (DIM3)*coords[2]);
                b(ii, jj, kk) = triple_t(ii - H1m + (DIM1)*coords[0] + B_ADD,
                    jj - H2m + (DIM2)*coords[1] + B_ADD,
                    kk - H3m + (DIM3)*coords[2] + B_ADD);
                c(ii, jj, kk) = triple_t(ii - H1m + (DIM1)*coords[0] + C_ADD,
    ...
    for (int ii = 0; ii < DIM1 + H1m + H1p; ++ii)
        for (int jj = 0; jj < DIM2 + H2m + H2p; ++jj)
            for (int kk = 0; kk < DIM3 + H3m + H3p; ++kk) {
                ...
                if (a(ii, jj, kk) != ta) {
                    passed = false;
                    file << "a " << a(ii, jj, kk) << " != " << ta << "\n";
                }

                if (b(ii, jj, kk) != tb) {
                    passed = false;
                    file << "b " << b(ii, jj, kk) << " != " << tb << "\n";
                }

                if (c(ii, jj, kk) != tc) {
                    passed = false;
                    file << "c " << c(ii, jj, kk) << " != " << tc << "\n";
                }
            }
