...
//todo simplify the following using the auto keyword from C++11
    ...
    bool result = verif.verify(coords, repository.utens_stage_ref(), repository.utens_stage(), halos);
#else
    verifier verif(1e-13, halo_size);
    bool result = verif.verify(coords, repository.utens_stage_ref(), repository.utens_stage());
#endif
