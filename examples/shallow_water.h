...
/*
  @file
  @brief This file shows an implementation of the "shallow water" stencil, with
...
/**@brief This traits class defined the necessary types and functions used by 
 *        all the functors defining the shallow water model
 */
...
struct bc_periodic : functor_traits {
  ...
  GT_FUNCTION void operator()(
      direction<I, minus_, K, typename boost::enable_if_c<I != minus_>::type>,
      DataField0& data_field0,
      uint_t i,
      uint_t j,
      uint_t k) const {
    // TODO use placeholders here instead of the storage
    data_field0.template get<Component, Snapshot>()[data_field0._index(i, j, k)] =
        data_field0.template get<Component, Snapshot>()[data_field0._index(
            i, data_field0.template dims<1>() - 1 - j, k)];
  }

  // periodic boundary conditions in J
  template <sign J, sign K, typename DataField0>
  GT_FUNCTION void operator()(direction<minus_, J, K>,
                              DataField0& data_field0,
                              uint_t i,
                              uint_t j,
                              uint_t k) const {
    // TODO use placeholders here instead of the storage
    data_field0
        .template get<Component, Snapshot>()[data_field0._index(i, j, k)] =
        data_field0.template get<Component, Snapshot>()[data_field0._index(
            data_field0.template dims<0>() - 1 - i, j, k)];
  }

  // default: do nothing
  template <sign I, sign J, sign K, typename P, typename DataField0>
  GT_FUNCTION void operator()(direction<I, J, K, P>,
                              DataField0& data_field0,
                              uint_t i,
                              uint_t j,
                              uint_t k) const {}

  // a drop over the water surface
  static constexpr float_type height = 2.;
  GT_FUNCTION
  static float_type droplet(uint_t const& i, uint_t const& j, uint_t const& k) {
    if (i > 0 && j > 0 && i < 4 && j < 4)
      return 1. +
             height * std::exp(-5 * (((i - 1) * dx()) * (((i - 1) * dx())) +
                                     ((j - 1) * dy()) * ((j - 1) * dy())));
    else
      return 1.;
  }
...
// 	GT_FUNCTION
// 	static float_type droplet(uint_t const& i, uint_t const& j, uint_t
// const& k){
...
// These are the stencil operators that compose the multistage stencil 
struct first_step_x : public functor_traits {
  /**GCC 4.8.2  bug: inheriting the 'using' aliases (or replacing the typedefs
     below with the 'using' syntax) from the base class produces an internal
  ...
  GT_FUNCTION static void Do(Evaluation const& eval, x_interval) {
    auto hx = alias<tmpx, comp>(0);
    auto h = alias<sol, comp>(0);
    auto ux = alias<tmpx, comp>(1);
    auto u = alias<sol, comp>(1);
    auto vx = alias<tmpx, comp>(2);
    auto v = alias<sol, comp>(2);
    ...
    eval(ux()) =
        eval(u(i + 1, j + 1) + u(j + 1) / 2. -
             ((pow<2>(u(i + 1, j + 1)) / h(i + 1, j + 1) +
               pow<2>(h(i + 1, j + 1)) * g() / 2.) -
              (pow<2>(u(j + 1)) / h(j + 1) + pow<2>(h(j + 1)) * (g() / 2.))) *
                 (dt() / (2. * dx())));

    eval(vx()) = eval((v(i + 1, j + 1) + v(j + 1)) / 2. -
...
struct second_step_y : public functor_traits {
  ...
  template <typename Evaluation>
  GT_FUNCTION static void Do(Evaluation const& eval, x_interval) {
    auto hy = alias<tmpy, comp>(0);
    auto h = alias<sol, comp>(0);
    auto uy = alias<tmpy, comp>(1);
    auto u = alias<sol, comp>(1);
    auto vy = alias<tmpy, comp>(2);
    auto v = alias<sol, comp>(2);
    ...
    eval(uy()) = eval((u(i + 1, j + 1) + u(i + 1)) / 2. -
                      (v(i + 1, j + 1) * u(i + 1, j + 1) / h(i + 1, j + 1) -
                       v(i + 1) * u(i + 1) / h(i + 1)) *
                          (dt() / (2 * dy())));

    eval(vy()) =
        eval(v(i + 1, j + 1) + v(i + 1) / 2. -
             ((pow<2>(v(i + 1, j + 1)) / h(i + 1, j + 1) +
               pow<2>(h(i + 1, j + 1)) * g() / 2.) -
              (pow<2>(v(i + 1)) / h(i + 1) + pow<2>(h(i + 1)) * (g() / 2.))) *
                 (dt() / (2. * dy())));
  }
};
  ...
  GT_FUNCTION static void Do(Evaluation const& eval, x_interval) {
    ...
    eval(sol()) =
        eval(sol() -
             ...
             (tmpy(c + 2, i /**/ - 1) - tmpy(c + 2, i - 1, j - 1)) *
                 (dt() / dy()) /**/);

    eval(sol(comp(1))) =
        eval(sol(c + 1) -
             (pow<2>(tmpx(c + 1, j - 1)) / tmpx(j - 1) +
              tmpx(j - 1) * tmpx(j - 1) * ((g() / 2.)) -
              (pow<2>(tmpx(c + 1, i - 1, j - 1)) / tmpx(i - 1, j - 1) +
               pow<2>(tmpx(i - 1, j - 1)) * ((g() / 2.)))) *
                 ((dt() / dx())) -
             (tmpy(c + 2, i - 1) * tmpy(c + 1, i - 1) / tmpy(i - 1) -
              tmpy(c + 2, i - 1, j - 1) * tmpy(c + 1, i - 1, j - 1) /
                  tmpy(i - 1, j - 1)) *
                 ((dt() / dy()))); /**/
    ...
    eval(sol(comp(2))) =
        eval(sol(comp(2)) -
             (tmpx(c + 1, j - 1) * tmpx(c + 2, j - 1) / tmpy(j - 1) -
              (tmpx(c + 1, i - 1, j - 1) * tmpx(c + 2, i - 1, j - 1)) /
                  tmpx(i - 1, j - 1)) *
                 ((dt() / dx())) -
             (pow<2>(tmpy(c + 2, i - 1)) / tmpy(i - 1) +
              pow<2>(tmpy(i - 1)) * ((g() / 2.)) -
              pow<2>(tmpy(c + 2, i - 1, j - 1)) / tmpy(i - 1, j - 1) +
              pow<2>(tmpy(i - 1, j - 1)) * ((g() / 2.))) *
                 ((dt() / dy())));
#else
    ...
    eval(sol()) = eval(sol() - (ux(j - 1) - ux(i - 1, j - 1)) * (dt() / dx()) -
    ...
    eval(sol(comp(2))) =
        eval(sol(comp(2)) -
             ...
             (pow<2>(vy(i - 1)) / hy(i - 1) + pow<2>(hy(i - 1)) * ((g() / 2.)) -
              (pow<2>(vy(i - 1, j - 1)) / hy(i - 1, j - 1) +
               pow<2>(hy(i - 1, j - 1)) * ((g() / 2.)))) *
                 ((dt() / dy())));
#endif
  }
};
...
#define BACKEND backend<Host, Naive>
    ...
    typedef gridtools::BACKEND::storage_type<float_type, layout_t>::type
        storage_type;

/* The nice interface does not compile today (CUDA 6.5) with nvcc (C++11 support
 * not complete yet)*/
#ifdef __CUDACC__
    // pointless and tedious syntax, temporary while thinking/waiting for an
    // alternative like below
    ...
    // Their order reflects the order the user will deal with them,
    ...
    ptr out1(tmpx.size()), out2(tmpx.size()), out3(tmpx.size()),
        out4(tmpy.size()), out5(tmpy.size()), out6(tmpy.size());
    ...
    // construction of the domain. The domain is the physical domain of the
    // problem, with all the physical fields that are used, temporary and not
    // It must be noted that the only fields to be passed to the constructor are
    // the non-temporary.
    // The order in which they have to be passed is the order in which they
    // appear scanning the placeholders in order. (I don't particularly like
    // this)
    ...
    auto shallow_water_stencil1 =
        make_computation<gridtools::BACKEND, layout_t>(
            ...
            (execute<forward>(), make_esf<first_step_x>(p_tmpx(), p_sol())),
            domain,
            coords1);
    ...
    auto shallow_water_stencil2 =
        make_computation<gridtools::BACKEND, layout_t>(
            make_mss  // mss_descriptor
            (execute<forward>(), make_esf<second_step_y>(p_tmpy(), p_sol())),
            domain,
            coords2);
    ...
    auto shallow_water_stencil = make_computation<gridtools::BACKEND, layout_t>(
        make_mss  // mss_descriptor
        (execute<forward>(), 
    ...
    for (; final_step::current_time < total_time; ++final_step::current_time) {
      ...
      // TODO: use placeholders here instead of the storage
      /*                                 component,snapshot */
      boundary_apply_gpu<bc_periodic<0, 0> >(halos, bc_periodic<0, 0>())
          .apply(sol);
      boundary_apply_gpu<bc_periodic<1, 0> >(halos, bc_periodic<1, 0>())
          .apply(sol);
#else
      // TODO: use placeholders here instead of the storage
      /*                             component,snapshot */
