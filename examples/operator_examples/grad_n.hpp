...
namespace operator_examples {
    ...
    bool test_grad_n( uint_t t_steps, char *mesh_file)
    {
        ...
        auto dual_edge_length_reciprocal = icosahedral_grid.make_storage<icosahedral_topology_t::edges, double>("dual_edge_length_reciprocal");
        ...
        typedef boost::mpl::vector<p_orientation_of_normal, p_dual_edge_length, p_weights, p_dual_edge_length_reciprocal>
                accessor_list_t_prep;

        gridtools::domain_type<accessor_list_t_prep> domain_prep(
                boost::fusion::make_vector(&orientation_of_normal, &dual_edge_length, &weights, &dual_edge_length_reciprocal)
        );
                ...
                gridtools::make_mss // mss_descriptor
                        (execute<forward>(),
                         gridtools::make_esf<grad_n_prep, icosahedral_topology_t, icosahedral_topology_t::edges >(
                                 p_orientation_of_normal(), p_dual_edge_length(), p_weights(), p_dual_edge_length_reciprocal())
                        )
        );
        ...
        typedef arg<1, edge_storage_type> p_dual_edge_length1;
        typedef arg<2, edge_storage_type> p_out_edges;

        typedef boost::mpl::vector<p_in_cells, p_dual_edge_length1, p_out_edges>
                accessor_list_t_flow_convention;

        gridtools::domain_type<accessor_list_t_flow_convention> domain_flow_convention(
                boost::fusion::make_vector(&in_cells, &dual_edge_length, &out_edges)
        );

        auto stencil_flow_convention = gridtools::make_computation<backend_t>(
                domain_flow_convention,
                grid_,
                gridtools::make_mss // mss_descriptor
                        (execute<forward>(),
                         gridtools::make_esf<grad_n_flow_convention, icosahedral_topology_t, icosahedral_topology_t::edges >(
                                 p_in_cells(), p_dual_edge_length1(), p_out_edges())
                        )
        );
        ...
        std::cout << "flow convention with constexpr: " << stencil_flow_convention_with_constexpr->print_meter() << std::endl;
#endif
        /*
         *  stencil of flow convention reciprocal
         */

        typedef arg<1, edge_storage_type> p_dual_edge_length_reciprocal1;

        typedef boost::mpl::vector<p_in_cells, p_dual_edge_length_reciprocal1, p_out_edges>
                accessor_list_t_reciprocal;

        gridtools::domain_type<accessor_list_t_reciprocal> domain_flow_convention_reciprocal(
                boost::fusion::make_vector(&in_cells, &dual_edge_length_reciprocal, &out_edges)
        );

        auto stencil_flow_convention_reciprocal = gridtools::make_computation<backend_t>(
                domain_flow_convention_reciprocal,
                grid_,
                gridtools::make_mss // mss_descriptor
                        (execute<forward>(),
                         gridtools::make_esf<grad_n_flow_convention_reciprocal, icosahedral_topology_t, icosahedral_topology_t::edges >(
                                 p_in_cells(), p_dual_edge_length_reciprocal1(), p_out_edges())
                        )
        );
        ...
        std::cout << "flow convention with reciprocal: " << stencil_flow_convention_reciprocal->print_meter() << std::endl;
#endif
        ...
        typedef arg<1, edges_of_cells_storage_type> p_weights1;

        typedef boost::mpl::vector<p_in_cells, p_weights1, p_out_edges>
                accessor_list_t_orientation_of_normal;

        gridtools::domain_type<accessor_list_t_orientation_of_normal> domain_orientation_of_normal(
                boost::fusion::make_vector(&in_cells, &weights, &out_edges)
        );


        auto stencil_orientation_of_normal = gridtools::make_computation<backend_t>(
                domain_orientation_of_normal,
                grid_,
                gridtools::make_mss // mss_descriptor
                        (execute<forward>(),
                         gridtools::make_esf<grad_n_orientation_of_normal, icosahedral_topology_t, icosahedral_topology_t::edges >(
                                 p_in_cells(), p_weights1(), p_out_edges())
                        )
        );
