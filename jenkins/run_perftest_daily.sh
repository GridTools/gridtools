...
for domain in 128 256; do
  ...
  if [[ -n "$results" ]]; then
    # plot history
    ./build/pyutils/driver.py -v -l $logfile perftest plot history -i $results -o report-$domain-full-history || { echo 'Plotting failed'; rm -rf $tmpdir; exit 1; }
    ./build/pyutils/driver.py -v -l $logfile perftest plot history -i $results -o report-$domain-last-history --limit=10 || { echo 'Plotting failed'; rm -rf $tmpdir; exit 1; }
  fi

  # plot backend comparison
  ./build/pyutils/driver.py -v -l $logfile perftest plot compare-backends -i $result -o report-$domain-backend-comparison || { echo 'Plotting failed'; rm -rf $tmpdir; exit 1; }
done
