...
for domain in 128 256; do
  ...
  ./build/pyutils/perfdriver.py -v -l $logfile run -s $domain $domain 80 -o $resultdir/$resultname || { echo 'Running failed'; rm -rf $tmpdir; exit 1; }
  ...
  for backend in cuda x86 mc; do
    ...
    if [[ -n "$results" ]]; then
      # plot history
      ./build/pyutils/perfdriver.py -v -l $logfile plot history -i $results -o history-$backend-$domain-full.png || { echo 'Plotting failed'; rm -rf $tmpdir; exit 1; }
      ./build/pyutils/perfdriver.py -v -l $logfile plot history -i $results -o history-$backend-$domain-last.png --limit=10 || { echo 'Plotting failed'; rm -rf $tmpdir; exit 1; }
    fi
  done
done
