...
def stella_path(grid_type, precision, backend):
    assert grid_type == 'strgrid'
    assert precision in {'float', 'double'}
    assert backend in {'cpu', 'cuda'}
    return system.stella_path(grid_type, precision, backend)


def stella_binary(grid_type, precision, backend):
    if backend == 'cuda':
        suffix = backend.upper()
    else:
        suffix = ''
    binary = os.path.join(stella_path(grid_type, precision, backend),
...
def stella_command(grid_type, precision, backend, stencil, size):
    binary = stella_binary(grid_type, precision, backend)
    ni, nj, nk = size
...
def gridtools_path(grid_type, precision, backend):
    assert grid_type in {'strgrid', 'icgrid'}
    assert precision in {'float', 'double'}
    assert backend in {'host', 'cuda'}
    return system.gridtools_path(grid_type, precision, backend)


def gridtools_binary(grid_type, precision, backend, stencil):
    binary = getattr(stencil, 'gridtools_' + backend.lower())
    binary = os.path.join(gridtools_path(grid_type, precision, backend),
                          binary)
    if not os.path.isfile(binary):
...
def gridtools_command(grid_type, precision, backend, stencil, size):
    binary = gridtools_binary(grid_type, precision, backend, stencil)
    ni, nj, nk = size
    halo = stencil.halo
...
def command(grid_type, precision, runtime, backend, stencil, size):
    assert runtime in {'stella', 'gridtools'}
    com = gridtools_command if runtime == 'gridtools' else stella_command
    return com(grid_type, precision, backend, stencil, size)
