...
int main(int argc, char **argv) {
    ...
    auto patterns = gridtools::parse_patterns(testing::FLAGS_gtest_filter);
    bool perf_mode = gridtools::init(argc, argv);
    if (perf_mode) {
        patterns.negatives.emplace_back("*/*_domain_size_*.*");
        if (!gridtools::cmdline_params::needs_verification()) {
