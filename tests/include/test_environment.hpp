...
#define GT_REGRESSION_TEST(name, env, backend)                                                       \
    ...
    using name##types_t = ::testing::Types<env::apply<backend, double, ::gridtools::cmdline_params>, \
        env::apply<backend, float, ::gridtools::cmdline_params>,                                     \
        env::apply<backend, double, ::gridtools::inlined_params<12, 33, 61>>,                        \
        env::apply<backend, double, ::gridtools::inlined_params<23, 11, 43>>>;                       \
    TYPED_TEST_CASE(name, name##types_t, ::gridtools::test_environment_names);                       \
...
namespace gridtools {
    ...
    struct test_environment<Halo, Axis, std::index_sequence<Is...>> {
        ...
        struct apply {
            ...
            // backend_supports_vertical_stencils
            static decltype(backend_supports_vertical_stencils(Backend())) supports_vertical_stencils() { return {}; }
            ...
            template <class T = FloatType, class U, std::enable_if_t<std::is_convertible<U const &, T>::value, int> = 0>
            static auto make_storage(U const &arg) {
