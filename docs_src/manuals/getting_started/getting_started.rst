...
Hence we can use ``easy_run`` simplified API. We pass the stencil, the backend object, the grid
...
We can not use ``easy_run`` API because now we should compose computation from three stencil calls. To achieve that
