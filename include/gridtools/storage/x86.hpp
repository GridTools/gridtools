...
namespace gridtools {
    namespace storage {
        namespace x86_impl_ {
             ...
             * E.g., make_layout< 5> will return following type: layout_map< 2, 3, 4, 0, 1 >.
        ...
        struct x86 {
            friend std::true_type storage_is_host_referenceable(x86) { return {}; }

            template <size_t Dims>
            friend typename x86_impl_::make_layout<Dims>::type storage_layout(
                x86, std::integral_constant<size_t, Dims>) {
                return {};
            }

            friend integral_constant<size_t, 1> storage_alignment(x86) { return {}; }

            template <class LazyType, class T = typename LazyType::type>
            friend auto storage_allocate(x86, LazyType, size_t size) {
