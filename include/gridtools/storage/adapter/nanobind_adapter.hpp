...
namespace gridtools {
    ...
    using _ssize_t = nanobind::ssize_t;
#else
    using _ssize_t = std::size_t;
#endif

    inline constexpr nanobind_sid_adapter_impl_::_ssize_t dynamic_size =
        static_cast<nanobind_sid_adapter_impl_::_ssize_t>(-1);

    // Use `dynamic_size` for dynamic stride, use an integral value for static
    // stride.
    template <_ssize_t... Values>
    using stride_spec = std::integer_sequence<_ssize_t, Values...>;
    ...
    template <_ssize_t... SpecValues, std::size_t... IndexValues>
    ...
    auto as_sid(
        nanobind::ndarray<T, nanobind::shape<Sizes...>, Args...> ndarray, Strides stride_spec_ = {}, StridesKind = {}) {
        using sid::property;
