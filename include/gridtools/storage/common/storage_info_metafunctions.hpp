...
namespace gridtools {
    ...
    struct handle_masked_dims {
        ...
        GT_FUNCTION static uint_t extend(Dim d) {
            GT_STATIC_ASSERT(
    ...
    GT_FUNCTION uint_t pad_dimensions(Int dimension) {
    ...
    struct get_strides<layout_map<LayoutArgs...>> {
      private:
        ...
        GT_FUNCTION static enable_if_t<N != -1 && N != layout_map_t::unmasked_length - 1, uint_t> get_stride(
            Dims... d) {
            return (get_value_from_pack(layout_map_t::template find<N + 1>(), d...)) * get_stride<N + 1>(d...);
        }

        template <int N, typename... Dims>
        GT_FUNCTION static enable_if_t<N == -1, uint_t> get_stride(Dims...) {
            return 0;
        }

        template <int N, typename... Dims>
        GT_FUNCTION static enable_if_t<N != -1 && N == layout_map_t::unmasked_length - 1, uint_t> get_stride(Dims...) {
            return 1;
        }
      ...
      public:
        ...
        GT_FUNCTION static array<uint_t, sizeof...(LayoutArgs)> get_stride_array(Dims... ds) {
