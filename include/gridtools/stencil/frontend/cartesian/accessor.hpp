    ...
    namespace stencil {
        namespace cartesian {
            namespace accessor_impl_ {
                template <size_t I>
                GT_FORCE_INLINE constexpr int_t pick_dimension() {
                    return 0;
                }

                template <size_t I, class... Ts>
                GT_FORCE_INLINE constexpr int_t pick_dimension(dimension<I> src, Ts &&...) {
                    return src.value;
                }

                template <size_t I, size_t J, class... Ts, std::enable_if_t<I != J, int> = 0>
                GT_FORCE_INLINE constexpr int_t pick_dimension(dimension<J> src, Ts... srcs) {
                ...
                GT_FORCE_INLINE constexpr int_t out_of_range_dim(dimension<I> obj) {
                    return obj.value;
                }

                template <size_t Dim, size_t I, std::enable_if_t<(I <= Dim), int> = 0>
                GT_FORCE_INLINE constexpr int_t out_of_range_dim(dimension<I>) {
                    return 0;
                }
                ...
                constexpr bool check_offsets(array<int_t, Dim> const &offsets) {
              ...
              public:
                ...
                template <class... Ts,
                    std::enable_if_t<sizeof...(Ts) < Dim && std::conjunction<std::is_convertible<Ts, int_t>...>::value,
                        int> = 0>
                GT_FORCE_INLINE constexpr accessor(Ts... offsets) : base_t({offsets...}) {
                ...
                GT_FORCE_INLINE constexpr accessor(typename accessor_impl_::just_int<Is>::type... offsets, Ts... zeros)
                    : base_t({offsets...}) {
                    assert((true && ... && (zeros == 0)));
                    assert(accessor_impl_::check_offsets<Extent>(*this));
                }
                ...
                GT_FORCE_INLINE constexpr accessor(dimension<J> src, dimension<Js>... srcs)
            ...
            class accessor<Id, Intent, Extent, 0, std::index_sequence<>> : public tuple<> {
              public:
                ...
                template <class... Ts, std::enable_if_t<accessor_impl_::are_ints<Ts...>::value, int> = 0>
                GT_FORCE_INLINE constexpr accessor(Ts... zeros) {
                    assert((true && ... && (zeros == 0)));
                }

                template <size_t J, size_t... Js>
                GT_FORCE_INLINE constexpr accessor(dimension<J> zero, dimension<Js>... zeros) {
                    assert(((zero.value == 0) && ... && (zeros.value == 0)));
                }
            };
