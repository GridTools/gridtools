    ...
    namespace core {
        ...
        struct esf_descriptor {
            static_assert(esf_impl_::has_param_list<EsfFunction>::type::value,
                "The type param_list was not found in a user functor definition. All user functors must have a type "
                "alias "
                "called \'param_list\', which is an instantiation of \'make_param_list\' accessors defined in the "
                "functor "
            ...
            static_assert(esf_impl_::check_param_list<typename EsfFunction::param_list>::value,
                "The list of accessors in a user functor (i.e. the param_list type to be defined on each functor) does "
                "not "
