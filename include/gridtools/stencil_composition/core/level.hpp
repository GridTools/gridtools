...
namespace gridtools {
    namespace core {
        ...
        struct level {
         ...
         (note that non negative splitter values simplify the index computation)");
            static_assert(-OffsetLimit <= Offset && Offset <= OffsetLimit, "check offset and splitter value ranges \n\
         (note that non negative splitter values simplify the index computation)");
        ...
        using level_to_index = level_index<_impl::calc_level_index(Level::splitter, Level::offset, Level::offset_limit),
            Level::offset_limit>;
