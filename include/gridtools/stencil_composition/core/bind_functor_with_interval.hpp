...
namespace gridtools {
    namespace core {
        ...
        /**
         *   Takes an elementary functor (Functor) and the level index (Index) as an input; deduces the interval that
         * should be used for the range from Index to Index::next and produces the functor where the deduced interval is
         * bound. I.e. the new functor has the apply method with a single argument and delegates to the original one.
         *
         *   Corner cases:
         ...
         *     - from the given Index we go left util we have found the level that matches as a lower bound. (Here we
         * use the fact that interval is convertible to its lower bound level). if we reach zero index, we fail.
         ...
         *
         *   TODO(anstaf): If we want to keep the strict requirements on interval overloads that we have now (no holes,
         * no overlaps), we need to add an additional predicate `is_valid_functor<Functor, AxisInterval>` just for the
         * user protection. The alternative is to clearly define overlap resolution rules for the user.
         */
