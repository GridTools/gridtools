    ...
    namespace cartesian {
        namespace call_interfaces_impl_ {
            ...
            GT_FUNCTION Res sum_offsets(Lhs &&lhs, Rhs rhs) {
                ...
                return tuple_util::host_device::generate<generators_t, Res>(wstd::forward<Lhs>(lhs), wstd::move(rhs));
            }
            ...
            GT_FUNCTION void evaluate_bound_functor(Eval &eval, Args args) {
                ...
                call_functor<Functor, Region>(make_evaluator(eval,
                    tuple_util::host_device::transform(get_transform_f<I, J, K>(), wstd::move(args), lazy_params_t())));
            }
        ...
        /** Main interface for calling stencil operators as functions.
           ...
           not called and will result in compilation error. The user is responsible for calling the proper apply
           overload) \tparam ReturnType Can be set or will be deduced from the first input argument \tparam Offi Offset
           along the i-direction (usually modified using at<...>) \tparam Offj Offset along the j-direction \tparam Offk
           Offset along the k-direction
        */
        ...
        class call {
          ...
          public:
            ...
            /**
             * With this interface a stencil function can be invoked and the offsets specified in the passed accessors
             * are used to access values, w.r.t the offsets specified in a optional  at<..> statement.
             */
            template <class Eval,
                class... Args,
         ...
         * variables of the type accessed (or converted to) by the accessor in the called function, where the results
         * should be obtained from. The values can also be used by the function as inputs.
         *
        ...
        struct call_proc {
            ...
            GT_FUNCTION static std::enable_if_t<sizeof...(Args) == meta::length<typename Functor::param_list>::value>
