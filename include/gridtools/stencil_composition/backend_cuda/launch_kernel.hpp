...
namespace gridtools {
    namespace cuda {
        ...
        GT_FORCE_INLINE void launch_kernel(dim3 const &blocks, Fun const &fun, size_t shared_memory_size = 0) {
            ...
            if (shared_memory_size)
                launch_kernel_impl_::kernel<num_threads, BlockSizeI, BlockSizeJ, Extent>
                    <<<blocks, dim3{BlockSizeI, BlockSizeJ + halo_lines, 1}, shared_memory_size>>>(fun);
            else
                launch_kernel_impl_::kernel<num_threads, BlockSizeI, BlockSizeJ, Extent>
