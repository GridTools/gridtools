...
namespace gridtools {
    ...
    static void fused_mss_loop(target::cuda, LocalDomainListArray const &local_domain_lists, const Grid &grid) {
        ...
        host::for_each<GT_META_CALL(meta::make_indices, boost::mpl::size<MssComponents>)>(
            mss_functor<MssComponents, Grid, LocalDomainListArray, target::cuda, execution_info_cuda>(
