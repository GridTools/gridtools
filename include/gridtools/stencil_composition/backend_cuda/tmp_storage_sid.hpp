...
namespace gridtools {
    ...
    namespace tmp_cuda_impl_ {
        inline hymap::keys<dim::i, dim::j>::values<int_t, int_t> origin_offset(int_t shift_i, int_t shift_j) {
            return {shift_i, shift_j};
        }

        template <class T, class Allocator>
        auto make_ptr_holder(Allocator &alloc, size_t num_elements) {
            return allocate(alloc, meta::lazy::id<T>{}, num_elements);
        }
    ...
    namespace tmp_cuda_impl_ {
        ...
        auto compute_strides(int_t n_blocks_i, int_t n_blocks_j) {
            return tuple_util::make<stride_keys_t::values>(1_c,
                BlockSizeI{},
