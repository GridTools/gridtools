    ...
    namespace cuda {
        ...
        struct backend {
            ...
            static void launch_msses(L<>, Grid const &grid, DataStores &&, PrevKernel prev_kernel = {}) {
                std::move(prev_kernel).launch_or_fuse(backend(), grid, no_kernel());
            }
            ...
            static void launch_msses(
                ...
                auto kernel = backend::make_mss_kernel<Deref, Mss>(grid, data_stores);
                auto fused_kernel = std::move(prev_kernel).launch_or_fuse(backend(), grid, std::move(kernel));
