...
namespace gridtools {
    namespace sid {
        namespace concept_impl_ {
             ...
             */
            template <class Sid>
            auto get_origin(Sid &obj) GT_AUTO_RETURN(sid_get_origin(obj));

            /**
             *  C-array specialization
             */
            template <class T, class Res = gridtools::add_pointer_t<gridtools::remove_all_extents_t<T>>>
            gridtools::enable_if_t<std::is_array<T>::value, host_device::simple_ptr_holder<Res>> get_origin(T &obj) {
                return {(Res)obj};
            }
            ...
            enable_if_t<!std::is_same<Res, not_provided>::value && !std::is_array<Sid>::value, Res> get_strides(
                Sid const &obj) {
            ...
            template <class T>
            enable_if_t<std::is_array<T>::value, typename get_array_strides<T>::type> get_strides(T const &) {
                return {};
            }
...
#undef GT_SID_DELEGATE_FROM_IMPL
        ...
        struct get_origin_f {
            template <class T>
            auto operator()(T &obj) const GT_AUTO_RETURN(get_origin(obj));
        };
    } // namespace sid
