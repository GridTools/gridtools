    ...
    namespace sid {
        namespace composite {
            namespace impl_ {
                struct deref_f {
                    template <class T>
                    GT_FUNCTION auto operator()(T const &obj) const GT_AUTO_RETURN(*obj);
                };

                struct call_f {
                    template <class T>
                    GT_FUNCTION auto operator()(T const &obj) const -> decay_t<decltype(obj())> {
                        return obj();
                    }
                };
                ...
                struct normalize_strides_f<L<Keys...>> {
                    ...
                    tuple<GT_META_CALL(normalized_stride_type, (Keys, decay_t<Strides>))...> operator()(
                        Sid const &sid) const {
              ...
              public:
                ...
                struct compressed {
                    ...
                    struct composite_entity {
                        ...
                        template <class... PtrHolders>
                        friend GT_FORCE_INLINE composite_ptr_holder<PtrHolders...> operator+(
