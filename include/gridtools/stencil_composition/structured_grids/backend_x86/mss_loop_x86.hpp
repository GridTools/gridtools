    ...
    namespace x86 {
        ...
        auto make_mss_loop(int_t k_total_length, KStep, Composite composite, LoopIntervals loop_intervals) {
            ...
            return [origin = sid::get_origin(composite) + offset,
                       ...
                       loop_intervals](int_t i_block, int_t j_block, int_t i_size, int_t j_size) {
                sid::ptr_diff_type<Composite> offset{};
                ...
                auto k_loop = [&loop_intervals, k_shift_back](auto &ptr, auto const &strides) {
                    tuple_util::for_each(
                        [&ptr, &strides, k_shift_back](auto loop_interval) {
                            for (int k = 0; k < loop_interval.count(); ++k) {
