...
namespace gridtools::fn {
    namespace unstructured_naive_impl_ {
        ...
        inline auto make_composite(pos_t const &pos) {
            return [&]<class OutTag, class InTags, class Out, class Ins>(OutTag, InTags, Out &out, Ins &&ins) {
    ...
    void fn_apply(
        unstructured<Sizes, Offsets, Horizontal> const &domain, meta::val<Stencil>, Output &output, Inputs inputs) {
        ...
        naive_apply<Stencil>(replace_horizontal<Horizontal>(domain.sizes),
            ...
            std::move(inputs),
            make_composite(pos),
            make_iterator<Horizontal>);
    }

    template <class ScanTag, class Sizes, class Offsets, class Horizontal, class Vertical, class Output, class Inputs>
    void fn_apply_scan(
        unstructured<Sizes, Offsets, Horizontal, Vertical> const &domain, ScanTag, Output &output, Inputs inputs) {
        using namespace unstructured_naive_impl_;
        pos_t pos;
        auto out = sid::rename_dimensions<Horizontal, unstructured_naive_impl_::hor>(output);
        naive_apply_scan<Vertical>(ScanTag(),
            replace_horizontal<Horizontal>(domain.sizes),
            replace_horizontal<Horizontal>(domain.offsets),
            out,
            std::move(inputs),
            make_composite(pos),
