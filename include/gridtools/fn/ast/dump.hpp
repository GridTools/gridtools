...
namespace gridtools::fn {
    namespace ast {
        namespace dump_impl_ {
            ...
            struct has_std_ostream_support : std::false_type {};

            template <class T>
            struct has_std_ostream_support<T,
                std::void_t<decltype(std::declval<std::ostream &>() << std::declval<T const &>())>> : std::true_type {};
            ...
            namespace lazy {
                ...
                struct make_f<builtin<Tag, Args...>, Arity> {
                    ...
                    using type = meta::if_<std::is_same<params_t, args_t>, Tag, fun<builtin<Tag, Args...>, Arity>>;
                };
            } // namespace lazy
            ...
            using make_fun = typename lazy::make_f<full_parse<parse<F, Args...>>,
                std::integral_constant<size_t, sizeof...(Args)>>::type;
        ...
        std::ostream &operator<<(std::ostream &s, builtin<Tag, Args...>) {
            using namespace dump_impl_;
            return s << Tag() << "(" << make_seq<meta::list<Args...>>() << ")";
        }
    } // namespace ast
    namespace builtins {
        template <class Tag, std::enable_if_t<!ast::dump_impl_::has_std_ostream_support<Tag>::value, int> = 0>
        std::ostream &operator<<(std::ostream &s, Tag) {
            auto &&name = boost::typeindex::type_id<Tag>().pretty_name();
            return s << name.substr(name.find_last_of(":") + 1);
        }

        template <class F>
        std::ostream &operator<<(std::ostream &s, ilift<F>) {
            return s << "ilift<" << F() << ">";
        }

        template <class F>
        std::ostream &operator<<(std::ostream &s, tlift<F>) {
            return s << "tlift<" << F() << ">";
        }

        template <size_t I>
        std::ostream &operator<<(std::ostream &s, tuple_get<I>) {
            return s << "tuple_get<" << I << ">";
        }

        template <auto... Vs>
        std::ostream &operator<<(std::ostream &s, shift<Vs...>) {
            return s << "shift<" << ast::dump_impl_::seq<Vs...> << ">";
        }
        ...
        std::ostream &operator<<(std::ostream &s,
            scan<IsBackward, meta::list<InitPass, InitGet>, meta::list<Pass, Get>, Prologues, Epilogues>) {
        ...
        std::ostream &operator<<(std::ostream &s, reduce<F, Init>) {
