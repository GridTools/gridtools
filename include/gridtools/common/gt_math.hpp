...
namespace gridtools {
    ...
    */
    ...
    struct gt_pow {
        ...
        constexpr GT_FUNCTION static Value apply(Value const &v) {
    ...
    struct gt_pow<0> {
        template <typename Value>
        constexpr GT_FUNCTION static Value apply(Value const &) {
            return 1.;
        }
    };
     ...
     * @return ceiled value
     */
    GT_FUNCTION static int gt_ceil(float num) {
    ...
    namespace math {
        ...
        template <typename Value>
        GT_FUNCTION impl_::minmax_return_type<Value> max(Value const &val0) {
            return val0;
        }

        template <typename Value, typename... OtherValues>
        GT_FUNCTION impl_::minmax_return_type<Value> max(
            Value const &val0, Value const &val1, OtherValues const &... vals) {
            return val0 > max(val1, vals...) ? val0 : max(val1, vals...);
        }

        template <typename Value>
        GT_FUNCTION impl_::minmax_return_type<Value> min(Value const &val0) {
            return val0;
        }

        template <typename Value, typename... OtherValues>
        GT_FUNCTION impl_::minmax_return_type<Value> min(
            Value const &val0, Value const &val1, OtherValues const &... vals) {
            return val0 > min(val1, vals...) ? min(val1, vals...) : val0;
        }
#else
        template <typename Value>
        GT_FUNCTION Value const &max(Value const &val0) {
            return val0;
        }

        template <typename Value>
        GT_FUNCTION Value const &max(Value const &val0, Value const &val1) {
            return val0 > val1 ? val0 : val1;
        }

        template <typename Value, typename... OtherValues>
        GT_FUNCTION Value const &max(Value const &val0, Value const &val1, OtherValues const &... vals) {
            return val0 > max(val1, vals...) ? val0 : max(val1, vals...);
        }

        template <typename Value>
        GT_FUNCTION Value const &min(Value const &val0) {
            return val0;
        }

        template <typename Value>
        GT_FUNCTION Value const &min(Value const &val0, Value const &val1) {
            return val0 > val1 ? val1 : val0;
        }

        template <typename Value, typename... OtherValues>
        GT_FUNCTION Value const &min(Value const &val0, Value const &val1, OtherValues const &... vals) {
            return val0 > min(val1, vals...) ? min(val1, vals...) : val0;
        }
#endif
