...
namespace gridtools{
    ...
    public:
        ...
        GT_FUNCTION
        constexpr int_t const& strides() const {return m_value.template strides<Id>();}

        template <typename Accessor>
        GT_FUNCTION
        constexpr int_t index(Accessor const& arg_) const {return   m_value._index(arg_);}

        template <typename Accessor, typename ... Pairs>
        GT_FUNCTION
        constexpr int_t index(accessor_mixed<Accessor, Pairs ...> const& arg_) const {return   m_value._index(arg_);}
    };
}//namespace gridtools
