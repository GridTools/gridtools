...
#ifdef CXX11_ENABLED
	/**
	 *	@brief function that forms a vector of caches that share the same cache type and input/output policy  (c++11 version)
...
#define _CREATE_CACHE(z, n, nil)																							\
    template < cache_type cacheType, cache_io_policy cacheIOPolicy, BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), typename T) >		\
    typename boost::mpl::transform< boost::mpl::vector< BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), T) >,							\
									detail::force_arg_resolution<cacheType, cacheIOPolicy>									\
	>::type cache(BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), T)) {																\
		typedef typename boost::mpl::transform<																				\
			boost::mpl::vector< BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), T) >,													\
			detail::force_arg_resolution<cacheType, cacheIOPolicy>															\
		>::type	res_type;																									\
		GRIDTOOLS_STATIC_ASSERT((boost::mpl::size<res_type>::value > 0), "Cannot build cache sequence without argument");	\
		return res_type();																									\
	}
