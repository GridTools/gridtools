...
#define _PARTMPL_(count, N, data)                  \
        typename data ## Type ## N

#define _PARDECL_(count, N, data)                  \
        data ## Type ## N data ## Value ## N


#define _RED_DESCR(z, n, nil)                                               \
    template < typename RedFunctor, typename BinOp, typename ReductionType, \
    ...
    reduction_descriptor< ReductionType, BinOp,                             \
        ...
        > > > >                                                             \
    make_reduction(const ReductionType initial_value,                       \
        BOOST_PP_ENUM(BOOST_PP_INC(n), _PARDECL_, Par) )            {       \
