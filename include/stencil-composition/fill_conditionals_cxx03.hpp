...
#define _PAIR_CONST_REF_(count, N, data)                  \
        data ## Type ## N const& data ## Value ## N
...
namespace gridtools{
    ...
    /**@brief storing the conditionals passed as arguments into a container (a set)
       ...
       for the following specialization. If the next multi stage stencil consumed is not a conditional, we jump to another state,
    ...
    /**@brief specialization for the case in which a branch is not a conditional
       ...
       This corresponds to another state, in the aformentioned state machine. If the subsequent multi stage stencil consumed is a
    ...
    static void fill_conditionals(ConditionalsSet& set_, First const& first_){
        fill_conditionals_set<typename boost::mpl::has_key<ConditionalsSet, typename if_condition_extract_index_t<First>::type>::type >::apply(set_, first_);
     }
