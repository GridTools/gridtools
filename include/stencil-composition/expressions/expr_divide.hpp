namespace gridtools{
    ...
    namespace expressions{
        ...
        template <typename IterateDomain, typename ArgType1, typename ArgType2
                  , typename boost::enable_if<
                        typename boost::mpl::or_<typename is_expr<ArgType2>::type
                                                 , typename is_accessor<ArgType2>::type>
                        , int >::type=0
                  >
        GT_FUNCTION
        auto static constexpr value(IterateDomain const& it_domain
                                    , expr_derivative<expr_divide<ArgType1, ArgType2> > const& arg)
        ...
        GT_FUNCTION
        auto static constexpr value(IterateDomain const& it_domain
                                    , expr_derivative<expr_divide<ArgType1, FloatType> > const& arg)
            -> decltype(it_domain(arg.first_operand ) ) {

            return it_domain(expr_derivative<ArgType1>(arg.first_operand.first_operand) / arg.first_operand.second_operand ) ;}
