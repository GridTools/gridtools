...
#define _META_ARRAY_VECTOR_(z,n,nil) \
    ...
    struct BOOST_PP_CAT(meta_array_vector , BOOST_PP_INC(BOOST_PP_INC(n))) { \
        typedef typename BOOST_PP_CAT(meta_array_vector , BOOST_PP_INC(n)) <typename boost::mpl::push_back<Vector , First>::type, BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), MssType)>::type type; \
    }; \
 \
    template<typename Vector, typename Mss1, typename Mss2, typename Cond, BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), typename MssType)> \
    struct BOOST_PP_CAT(meta_array_vector , BOOST_PP_INC(BOOST_PP_INC(n))) <Vector, condition<Mss1, Mss2, Cond>, BOOST_PP_ENUM_PARAMS(BOOST_PP_INC(n), MssType) > { \
        typedef condition< \
                           typename BOOST_PP_CAT(meta_array_vector , BOOST_PP_INC(BOOST_PP_INC(n))) <Vector \
