...
public:
    ...
    template<short_t snapshot, short_t component, typename storage_type>
    bool verify(storage_type const& field1, storage_type const& field2) const
    {
        ...
        for(gridtools::uint_t i=m_halo_size; i < idim-m_halo_size; ++i)
        {
            for(gridtools::uint_t j=m_halo_size; j < jdim-m_halo_size; ++j)
            {
                ...
                {
                    typename storage_type::value_type expected = field1.template get_value<snapshot, component>(i,j,k);
                    typename storage_type::value_type actual = field2.template get_value<snapshot, component>(i,j,k);
