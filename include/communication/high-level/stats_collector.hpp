...
// high-level events are used to get an understanding of the overll communication pattern
    enum ExchangeEventType {ee_pack,
                            ee_unpack,
                            ee_exchange,
                            ee_start_exchange,
                            ee_wait,
...
public:
    ...
    static collector* instance(){
        return instance_ ? instance_ : (instance_ = new collector);
    }
    ...
    const_exchange_iterator exchange_begin() const {
        return exchange_events_.begin();
    }
    const_exchange_iterator exchange_end() const {
        return exchange_events_.end();
    }
    ...
    const_event_iterator events_begin() const {
        return events_.begin();
    }
    const_event_iterator events_end() const {
        return events_.end();
    }
    ...
    const_pattern_iterator patterns_begin() const {
    ...
    void evaluate(S &stream, int level=0) const {
        ...
        time_table[ee_pack] = time_table[ee_unpack] = time_table[ee_wait]
                            = time_table[ee_exchange] = time_table[ee_start_exchange]
                            = 0.;
        ...
        for(std::map<int,int>::const_iterator it=pattern_map.begin();
            it!=pattern_map.end();
            it++)
        {
        ...
        if(!rank){
            stream << ".-----------------------------------------------------------------------------------."
                   << std::endl;
            sprintf(str, "|%7s |%14s%14s%14s%14s |%14s  |", "pattern", "pack", "wait", "start_exchg", "exchg", "TOTAL");
            stream << str << std::endl;
            stream << ".-----------------------------------------------------------------------------------."
                   << std::endl;
        }
        ...
        int idx=0;
        for( std::map<int, PatternTimeTable>::iterator it=pattern_times.begin();
             it!=pattern_times.end();
             it++ )
        {
            ...
            if(!rank){
                sprintf(str, "|%7d |%14.8f%14.8f%14.8f%14.8f | %14.8f |"
                           , idx++
        ...
        MPI_Reduce(local_times, mean_times, 5, MPI_DOUBLE, MPI_SUM, 0, comm_);
        if(!rank){
            sprintf(str, "%8s |%14.8f%14.8f%14.8f%14.8f | %14.8f |"
                       , "SUM_MEAN"
                       , mean_times[0] / double(size)
                       , mean_times[1] / double(size)
                       , mean_times[2] / double(size)
        ...
        if(!rank){
            sprintf(str, "%7s  |%14.8f%14.8f%14.8f%14.8f | %14.8f |"
                       , "SUM_MIN"
                       , mean_times[0]
                       , mean_times[1]
                       , mean_times[2]
    ...
    void print(S &stream, int level=1) const {
        ...
        if( level==2 ){
            ...
            sprintf(str, "%8s%6s%14s%5s%6s%12s%11s%13s", "pattern", "idx", "type", "rank", "tag", "size", "t_start", "t_duration");
            stream << str << std::endl;
            stream << "-------------------------------------------------------------------------------" << std::endl;
            ...
            for(const_event_iterator it = events_begin(); it != events_end(); it++, idx++){
                sprintf( str, "%8d%6d%14s%5d%6d%12d%11.5f%13.10f",
                         it->pattern, idx, event_labels[it->type].c_str(),
                         it->other_rank, it->tag, it->message_size,
        ...
        if( level>=1 ){
            ...
            for(const_exchange_iterator it = exchange_begin(); it != exchange_end(); it++, idx++){
                sprintf( str, "%8d%6d%14s%7d%11.5f%11.8f",
                         it->pattern,
                         idx,
