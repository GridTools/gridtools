...
namespace gridtools {
    ...
    struct do_method_lookup_map_add
    {
        ...
        BOOST_STATIC_ASSERT((is_sub_interval<TLoopInterval, NextDoInterval>::value));
    ...
    struct compute_functor_do_method_lookup_map
    {
        // check the collection are not empty
        BOOST_STATIC_ASSERT(!boost::mpl::empty<TDoMethods>::value);
        BOOST_STATIC_ASSERT(!boost::mpl::empty<TLoopIntervals>::value);
