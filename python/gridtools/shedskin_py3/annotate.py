...
'''
import re
from compiler.ast import Num, AssTuple, AssList, Assign, AugAssign, \
...
def annotate(gx):
    ...
    for module in list(gx.modules.values()):
        if module.builtin:
            continue
        mv = module.mv
        merge = dict((k, v) for k, v in list(gx.merged_inh.items()) if (k, 0, 0) in gx.cnode and inode(gx, k).mv == mv)
        source = open(module.filename).readlines()

        # --- constants/names/attributes
        for expr in merge:
            if isinstance(expr, (Num, Name)):
        ...
        # --- instance variables
        funcs = list(mv.funcs.values())
        for cl in list(mv.classes.values()):
            labels = [var.name + ': ' + nodetypestr(gx, var, cl, False, mv=mv) for var in list(cl.vars.values()) if var in merge and merge[var] and not var.name.startswith('__')]
            if labels:
                paste(gx, source, cl.node, ', '.join(labels), mv)
            funcs += list(cl.funcs.values())
