...
from compiler.ast import Num, AssTuple, AssList, From, Add, Stmt, AssAttr, \
...
class GenerateVisitor(ASTVisitor):
    ...
    def insert_consts(self, declare):  # XXX ugly
        ...
        for (i, line) in enumerate(lines):
            ...
            if i == j:
                ...
                for (node, name) in list(self.consts.items()):
            ...
            if i == j:
                ...
                for number in todolist:
                    if self.mergeinh[todo[number]]:  # XXX
                        ...
                        self.start('    ' + name + ' = ')
                        if isinstance(todo[number], Num) and isinstance(todo[number].value, str) and len(todo[number].value) == 1:
    ...
    def fwd_class_refs(self):
        ...
        for _module in self.module.prop_includes:
            ...
            for cl in list(_module.mv.classes.values()):
    ...
    def include_files(self):
        ...
        while changed:
            ...
            for module in list(includes):
                ...
                includes.update(list(module.mv.imports.values()))
                includes.update(list(module.mv.fake_imports.values()))
        ...
        for include in includes:
            include.deps = set()
        for include in includes:
            for cl in list(include.mv.classes.values()):
                if cl.bases:
    ...
    def group_declarations(self, pairs):
        ...
        result = []
        for (type, names) in list(group.items()):
            names.sort()
            if type.endswith('*'):
    ...
    def output(self, text):
        print(self.indentation + text, file=self.out)
    ...
    def module_hpp(self, node):
        ...
        print('#ifndef __' + define, file=self.out)
        print('#define __' + define + '\n', file=self.out)

        # --- namespaces
        print('using namespace __shedskin__;', file=self.out)
        for n in self.module.name_list:
            print('namespace __' + n + '__ {', file=self.out)
        print(file=self.out)
        ...
        for child in node.node.getChildNodes():
            if isinstance(child, ClassNode):
                ...
                print('class ' + self.cpp_name(cl) + ';', file=self.out)
        print(file=self.out)
        ...
        if self.module != self.gx.main_module:
            print('void __init();', file=self.out)
        for child in node.node.getChildNodes():
        ...
        if self.gx.extension_module:
            print('extern "C" {', file=self.out)
            pyinit_func(self)
            print('}', file=self.out)

        for n in self.module.name_list:
            print('} // module namespace', file=self.out)
        ...
        print('#endif', file=self.out)

    def defaults(self, declare):
        if self.module.mv.defaults:
            extern = ['', 'extern '][declare]
            for default, (nr, func, func_def_nr) in list(self.module.mv.defaults.items()):
                formal = func.formals[len(func.formals) - len(func.defaults) + func_def_nr]
                var = func.vars[formal]
                print(extern + typestr(self.gx, self.mergeinh[var], func, mv=self.mv) + ' ' + ('default_%d;' % nr), file=self.out)
            print(file=self.out)
    ...
    def rich_comparison(self):
        ...
        for cl in list(self.mv.classes.values()):
        ...
        if cmp_cls or lt_cls or gt_cls or le_cls or ge_cls:
            print('namespace __shedskin__ { /* XXX */', file=self.out)
            for cl in cmp_cls:
                t = '__%s__::%s *' % (self.mv.module.ident, self.cpp_name(cl))
                print('template<> inline __ss_int __cmp(%sa, %sb) {' % (t, t), file=self.out)
                print('    if (!a) return -1;', file=self.out)
                if '__eq__' in cl.funcs:
                    print('    if(a->__eq__(b)) return 0;', file=self.out)
                if '__lt__' in cl.funcs:
                    print('    return (a->__lt__(b))?-1:1;', file=self.out)
                elif '__gt__' in cl.funcs:
                    print('    return (a->__gt__(b))?1:-1;', file=self.out)
                else:
                    print('    return __cmp<void *>(a, b);', file=self.out)
                print('}', file=self.out)
    ...
    def rich_compare(self, cls, msg, fallback_msg):
        for cl in cls:
            t = '__%s__::%s *' % (self.mv.module.ident, self.cpp_name(cl))
            print('template<> inline __ss_bool __%s(%sa, %sb) {' % (msg, t, t), file=self.out)
            # print >>self.out, '    if (!a) return -1;' # XXX check
            print('    return b->__%s__(a);' % fallback_msg, file=self.out)
            print('}', file=self.out)

    def module_cpp(self, node):
        print('#include "builtin.hpp"\n', file=self.out)
        ...
        for child in node.node.getChildNodes():
            if isinstance(child, From) and child.modname != '__future__':
                ...
                for (name, pseudonym) in child.names:
                    ...
                    if name == '*':
                        for func in list(module.mv.funcs.values()):
                            if func.cp:  # XXX
                                print(using + self.cpp_name(func) + ';', file=self.out)
                        for cl in list(module.mv.classes.values()):
                            print(using + self.cpp_name(cl) + ';', file=self.out)
                        ...
                        if name in module.mv.funcs:
                            ...
                            print(using + self.namer.nokeywords(name) + ';', file=self.out)
        print(file=self.out)
        ...
        for child in node.node.getChildNodes():
            if isinstance(child, FunctionNode):
                ...
                for (name, pseudonym) in child.names:
                    pseudonym = pseudonym or name
                    if name == '*':
                        for var in list(module.mv.globals.values()):
    ...
    def visit_discard(self, node, func=None):
        if isinstance(node.expr, Num) and node.expr.value is None:  # XXX merge with visitStmt
            pass
        elif isinstance(node.expr, Num) and type(node.expr.value) == str:
    ...
    def do_main(self):
        modules = list(self.gx.modules.values())
        if any(module.builtin and module.ident == 'sys' for module in modules):
            print('int main(int __ss_argc, char **__ss_argv) {', file=self.out)
        else:
            print('int main(int, char **) {', file=self.out)
        self.do_init_modules()
        print('    __shedskin__::__start(__%s__::__init);' % self.module.ident, file=self.out)
        print('}', file=self.out)

    def do_init_modules(self):
        print('    __shedskin__::__init();', file=self.out)
        for module in sorted(list(self.gx.modules.values()), key=lambda x: x.import_order):
            if module != self.gx.main_module and module.ident != 'builtin':
                if module.ident == 'sys':
                    if self.gx.extension_module:
                        print('    __sys__::__init(0, 0);', file=self.out)
                    else:
                        print('    __sys__::__init(__ss_argc, __ss_argv);', file=self.out)
                else:
                    print('    ' + module.full_path() + '::__init();', file=self.out)
    ...
    def copy_method(self, cl, name, declare):
        ...
        if not declare:
            ...
            for var in list(cl.vars.values()):
    ...
    def class_hpp(self, node):
        ...
        for func in list(cl.funcs.values()):
    ...
    def class_cpp(self, node):
        ...
        if cl.parent.vars:  # XXX merge with visitModule
            for var in list(cl.parent.vars.values()):
    ...
    def visitFor(self, node, func=None):
        ...
        if is_fastfor(node):
            ...
            print(self.line + ',' + tail + ')', file=self.out)
    ...
    def do_fastzip2(self, node, func, genexpr):
        ...
        print(self.line + tail1 + tail2 + ')', file=self.out)
        self.indent()
    ...
    def func_pointers(self):
        for func in list(self.mv.lambdas.values()):
            ...
            print('typedef %s(*lambda%d)(' % (rettype, func.lambdanr) + ', '.join(argtypes) + ');', file=self.out)
        print(file=self.out)
    ...
    def visitFunction(self, node, parent=None, declare=False):
        ...
        # --- function body
        for fake_unpack in list(func.expand_args.values()):
    ...
    def visitBinary(self, left, right, middle, inline, func=None):  # XXX cleanup please
        ...
        if inline in ['+', '-'] and isinstance(right, Num) and isinstance(right.value, complex):
            if floattype.intersection(ltypes) or inttype.intersection(ltypes):
    ...
    def visitCallFunc(self, node, func=None, argtypes=None):
        ...
        if self.library_func(funcs, 'array', 'array', '__init__'):
            if not node.args or not isinstance(node.args[0], Num) or node.args[0].value not in 'cbBhHiIlLfd':
        ...
        if node.node in self.mergeinh and [t for t in self.mergeinh[node.node] if isinstance(t[0], Function)]:  # anonymous function
            ...
            if ident == '__getitem__' and isinstance(node.args[0], Num) and node.args[0].value in (0, 1) and self.only_classes(objexpr, ('tuple2',)):
                ...
                if [n for n in node.nodes if isinstance(n, AssTuple)]:  # XXX a,b=d[i,j]=..?
                    for child in node.expr.nodes:
                        if not (child, 0, 0) in self.gx.cnode:  # (a,b) = (1,2): (1,2) never visited
                            continue
                        if not isinstance(child, Num) and not (isinstance(child, Name) and child.name == 'None'):
                            self.start(self.mv.tempcount[child] + ' = ')
                            self.visit(child, func)
                            self.eol()
            elif not isinstance(node.expr, Num) and not (isinstance(node.expr, Name) and node.expr.name == 'None'):
    ...
    def do_lambdas(self, declare):
        for l in list(self.mv.lambdas.values()):
    ...
    def do_listcomps(self, declare):
        for (listcomp, lcfunc, func) in self.mv.listcomps:  # XXX cleanup
            ...
            genexpr = listcomp in list(self.gx.genexp_to_lc.values())
            if declare:
    ...
    def local_defs(self, func):
        pairs = []
        for (name, var) in list(func.vars.items()):
    ...
    def visitListComp(self, node, func=None):
        ...
        self.line = temp
        if node in list(self.gx.genexp_to_lc.values()):
            self.append('new ')
...
def generate_code(gx):
    for module in list(gx.modules.values()):
