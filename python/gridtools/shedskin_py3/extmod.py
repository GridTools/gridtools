...
def do_add_globals(gx, gv, classes, __ss_mod):
    # global variables
    for var in supported_vars(gx, gv, list(gv.mv.globals.values())):
...
def do_extmod_method(gx, gv, func):
    ...
    print('        PyErr_SetString(__to_py(e), ((e->message)?(e->message->unit.c_str()):""));', file=gv.out)
...
def do_extmod(gx, gv):
    ...
    # global functions
    funcs = supported_funcs(gx, gv, list(gv.module.mv.funcs.values()))
    for func in funcs:
...
def do_extmod_class(gx, gv, cl):
    ...
    funcs = supported_funcs(gx, gv, list(cl.funcs.values()))
    vars = supported_vars(gx, gv, list(cl.vars.values()))
...
def exported_classes(gx, gv, warns=False):
    classes = []
    for cl in list(gv.module.mv.classes.values()):
        if def_class(gx, 'Exception') in cl.ancestors():
            if warns:
