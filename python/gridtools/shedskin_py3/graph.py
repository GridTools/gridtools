...
import sys
"""
from ast import Num, AssTuple, AssList, From, Add, ListCompFor, \
...
def slice_nums(nodes):
    ...
    for i, n in enumerate(nodes):
        if not n or (isinstance(n, Num) and n.value is None):
            nodes2.append(Num(0))
        else:
            nodes2.append(n)
            x |= (1 << i)
...
class ModuleVisitor(NodeVisitor):
    ...
    def dispatch(self, node, *args):
        if (node, 0, 0) not in self.gx.cnode:
            NodeVisitor.dispatch(self, node, *args)
    ...
    def visitStmt(self, node, func=None):
        ...
        for b in node.nodes:
            ...
            if isinstance(b, Discard) and isinstance(b.expr, Num) and type(b.expr.value) == str:
    ...
    def visitModule(self, node):
        ...
        # --- register classes
        for cl in list(getmv().classes.values()):
        ...
        for cl in list(self.classes.values()):
            for ancestor in cl.ancestors_upto(None)[1:]:
                ...
                for func in list(ancestor.funcs.values()):
    ...
    def visitFrom(self, node, parent=None):
        ...
        for name, pseudonym in node.names:
            if name == '*':
                ...
                for import_name, import_module in list(module.mv.imports.items()):
                ...
                for name, extvar in list(module.mv.globals.items()):
    ...
    def visitCompare(self, node, func=None):
        ...
        for term in node.ops[:-1]:
            if not isinstance(term[1], (Name, Num)):
    ...
    def visitFor(self, node, func=None):
        ...
        if isinstance(node.assign, AssName):
            ...
            fakefunc2 = CallFunc(Getattr(node.assign.expr, '__setattr__'), [Num(node.assign.attrname), fakefunc])
    ...
    def visitListComp(self, node, func=None):
        ...
        # node type
        if node in list(self.gx.genexp_to_lc.values()):  # converted generator expression
    ...
    def visitReturn(self, node, func):
        ...
        if not (isinstance(node.value, Num) and node.value.value is None):
            newnode = CNode(self.gx, node, parent=func, mv=getmv())
            self.gx.types[newnode] = set()
    ...
    def assign_pair(self, lvalue, rvalue, func):
        ...
        # expr.attr = expr
            ...
            fakefunc = CallFunc(Getattr(lvalue.expr, '__setattr__'), [Num(lvalue.attrname), rvalue])
            self.visit(fakefunc, func)
    ...
    def tuple_flow(self, lvalue, rvalue, func=None):
        ...
        for (i, item) in enumerate(lvalue):
            ...
            fakefunc = CallFunc(FakeGetattr3(rvalue, '__getitem__'), [Num(i)])
    ...
    def visitClass(self, node, parent=None):
        ...
        if not newclass.mv.module.builtin and not '__hash__' in newclass.funcs:
            self.visit(FunctionNode(None, '__hash__', ['self'], [], 0, None, Return(Num(0)), []), newclass)
    ...
    def visitGetattr(self, node, func=None, callfunc=False):
        ...
        fakefunc = CallFunc(FakeGetattr(node.expr, '__getattr__'), [Num(node.attrname)])
        self.visit(fakefunc, func)
    ...
    def visitConst(self, node, func=None):
        ...
        map = {int: 'int_', str: 'str_', float: 'float_', type(None): 'none', int: 'int_', complex: 'complex'}  # XXX 'return' -> Return(Num(None))?
