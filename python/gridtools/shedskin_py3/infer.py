...
import random
import sys
from ast import Num, AST, Attribute, keyword, Call, Dict, List, Tuple, ListComp, Not, Compare, Name

from gridtools.shedskin import error
from . import graph
...
def called(func):
    return bool([cpas for cpas in list(func.cp.values()) if cpas])
...
def analyze_callfunc(gx, node, node2=None, merge=None):  # XXX generate target list XXX uniform Variable system! XXX node2, merge?
    ...
    # method call
    if isinstance(node.node, Attribute):
...
def class_copy(gx, cl, dcpa):
    for var in list(cl.vars.values()):  # XXX
...
def redirect(gx, c, dcpa, func, callfunc, ident, callnode, direct_call, constructor):
    ...
    # array
    if constructor and ident == 'array' and isinstance(callfunc.args[0], Num):
    ...
    if (isinstance(callfunc.node, Attribute) and callfunc.node.identifier == '__getitem__' and
        isinstance(callfunc.args[0], Num) and callfunc.args[0].value in (0, 1) and
    ...
    # property
    if isinstance(callfunc.node, Attribute) and callfunc.node.identifier in ['__setattr__', '__getattr__']:
...
def connect_getsetattr(gx, func, callnode, callfunc, dcpa, worklist):
    if (isinstance(callfunc.node, Attribute) and callfunc.node.identifier in ['__setattr__', '__getattr__'] and
...
def ifa_split_vars(gx, cl, dcpa, vars, nr_classes, classes_nr, split, allcsites):
    for (varnum, var) in enumerate(vars):
        ...
        if len(prt) > 1:
            if DEBUG(gx, 3):
                print('IFA partition csites:', list(prt.values())[0])
...
def ifa_split_no_confusion(gx, cl, dcpa, varnum, classes_nr, nr_classes, csites, emptycsites, allnodes, split):
    ...
    items = list(subtype_csites.items())
    if not others:
        items = items[1:]
    for subtype, csites in subtype_csites.items():  # XXX items?
    ...
    if DEBUG(gx, 3) and subtype_csites:
        print('IFA found simple split', list(subtype_csites.keys()))
...
def ifa_class_types(gx, cl, vars):
    ...
    for dcpa in range(1, cl.dcpa):
        ...
        if DEBUG(gx, 3) and [x for x in attr_types if x]:
            print('IFA', str(dcpa) + ':', list(zip([var.name for var in vars], list(map(list, attr_types)))))
...
def ifa_confluence_point(node, creation_points):
    ...
    if len(node.in_) > 1 and isinstance(node.thing, Variable):
        for csite in node.csites:
            occ = [csite in crpoints for crpoints in list(creation_points.values())].count(True)
            if occ > 1:
                return True
    return False
...
def update_progressbar(gx, perc):
    if not gx.silent:
        print('\r%s%d%%' % (int(perc * 32) * '*', 100 * perc), end=' ')
        if DEBUG(gx, 1):
            print()
        else:
            sys.stdout.flush()
            ...
            for n in nodes:
                ...
                if parent:
                    if n.dcpa in parent.cp:
                        for cart, cpa in list(parent.cp[n.dcpa].items()):  # XXX not very fast
                            if cpa == n.cpa:
...
def backup_network(gx):
    beforetypes = {}
    for node, typeset in list(gx.types.items()):
...
def restore_network(gx, backup):
    ...
    gx.types = {}
    for node, typeset in list(beforetypes.items()):
...
def analyze(gx, module_name):
    ...
    for node in gx.merged_inh:
        if isinstance(node, AST) and not isinstance(node, Attribute) and isinstance (node.expr_context, Assign) and not inode(gx, node).mv.module.builtin:
