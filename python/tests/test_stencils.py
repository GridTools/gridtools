...
class IfStatementTest (unittest.TestCase):
    ...
    def setUp (self):
        ...
        self.stencilb = AdditionalIfStatement      (self.domain)
        self.stencil2 = IfStatementOpIsFailure     (self.domain)
        self.stencil3 = IfStatementOpIsNotFailure  (self.domain)
        self.stencil4 = IfStatementOpNotInFailure  (self.domain)
        self.stencil5 = IfStatementOpInFailure     (self.domain)
    ...
    def test_game_of_life (self):
        ...
        print("Results:  The Python and C++ runs are the same, right? %s" % np.array_equal (self.out_X,out_cpp))
        self.assertTrue (np.array_equal (self.out_X,out_cpp))


    def test_run_additional_if_statement_tests (self):
        self.stencilb.backend = 'c++'
        self._runb ( )
        out_cpp = np.copy (self.out_X)                        # Save copy of C++ run's output
        self.stencilb.backend = 'python'
        self.out_X = np.copy (np.asfortranarray(self.in_X))   # Copy original input back but with Fortran layout
        self._runb ( )                                        # Now run using the Python backend
        print("Results:  The Python and C++ runs are the same, right? %s" % np.array_equal (self.out_X,out_cpp))
        self.assertTrue (np.array_equal (self.out_X,out_cpp))
