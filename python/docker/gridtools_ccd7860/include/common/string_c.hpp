...
*/
    ...
    struct print{
        ...
        template< typename ... S >
        GT_FUNCTION
        static void apply(const char* first, S ... s ){
            printf("%s", first) ;
            apply(s...);
        }

        template<typename IntType, typename ... S, typename boost::enable_if<typename boost::is_integral<IntType>::type, int >::type=0 >
        GT_FUNCTION
        static void apply(IntType first, S ... s ){
            printf("%d", first) ;
            apply(s...);
        }

        template<typename FloatType, typename ... S , typename boost::enable_if<typename boost::is_floating_point<FloatType>::type, int >::type=0 >
        GT_FUNCTION
        static void apply(FloatType first, S ... s ){
            printf("%f", first);
            apply(s...);
        }
    };
