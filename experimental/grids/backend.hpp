...
namespace gridtools {
    ...
    public:
        ...
        template <ushort_t NColors, char const* Name, typename X, typename LayoutMap>
        struct _storage_type<location_type<2, NColors, Name>, X, LayoutMap> {
            using type = base_storage<wrap_pointer<double>, LayoutMap, location_type<2, NColors, Name> >;
        };
    ...
    private:
        ...
        static
        void
        dispatch_on_locationtype(location_type<2, NColors, Name>,
                                 Accessor & acc,
                                 Computation const x,
                                 Coords const & coords)
        {
            const auto low_bounds = acc.grid().ll_indices({coords.lb0, coords.lb1}, location_type<2, NColors, Name>());
            const auto high_bounds = acc.grid().ll_indices({coords.ub0-1, coords.ub1-1}, location_type<2, NColors, Name>());
            std::cout << "Low bounds  " << low_bounds << std::endl;
            std::cout << "High bounds " << high_bounds << std::endl;
            std::cout << "Iteration space on Edges "
                      << "from " << low_bounds[0] << " to " <<  high_bounds[0] << " "
            ...
            for (int i = low_bounds[0]; i <= high_bounds[0]; ++i) {
                for (int j = low_bounds[1]; j <= high_bounds[1]; ++j) { // they should always be 0 and 1 for cells
                    acc.template set_ll_ijk<location_type<2, NColors, Name> >(i, j, low_bounds[2]);
                    for (int k = low_bounds[2]; k <= high_bounds[2]; ++k) {
                        typename decltype(x)::functor()(acc);
                        acc.inc_ll_k();
                    }
