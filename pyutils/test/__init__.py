...
def run(run_mpi_tests, verbose_ctest):
    if run_mpi_tests:
        runtools.salloc(
            _ctest(only="mpi", verbose=verbose_ctest),
            log_output=log.info,
        ...
        runtools.srun(
            _ctest(exclude="mpi", verbose=verbose_ctest),
            log_output=log.info,
            cwd=buildinfo.binary_dir,
        )
...
def run_perftests():
    runtools.srun(
        [os.path.join("tests", "regression", "perftests")],
        log_output=log.info,
        cwd=buildinfo.binary_dir,
    )
...
def compile_and_run_examples(build_dir, verbose_ctest):
    ...
    runtools.srun(_ctest(verbose=verbose_ctest), log_output=log.info, cwd=build_dir)
