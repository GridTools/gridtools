...
async def _run_async(command, log_output, **kwargs):
    ...
    returncode, stdout, stderr = await asyncio.gather(
        process.wait(), read_output(process.stdout), read_output(process.stderr)
    )

    if returncode != 0:
        commstr = " ".join(f'"{c}"' for c in command)
        log.error(f"{commstr} finished with exit code {returncode} and message", stderr)
...
def srun(command, use_mpi_config=False, **kwargs):
    if _slurm_available() and env.run_with_slurm():
